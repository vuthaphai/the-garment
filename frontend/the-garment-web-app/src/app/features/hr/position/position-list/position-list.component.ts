...
import { Position } from '@core/models/hr.model';
...
export class PositionListComponent implements OnInit {
  ...
  onSearch(event: Event) { this.load((event.target as HTMLInputElement).value); }

  openForm(item?: Position) {
  ...
  save() {
    ...
    action.subscribe({
      next: () => { this.notify.success('Saved'); this.cancel(); this.load(); },
