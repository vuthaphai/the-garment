...
var AuthService = class _AuthService {
  constructor(http, router) {
    this.http = http;
    this.router = router;
    this.TOKEN_KEY = "tg_token";
    this.USER_KEY = "tg_user";
    this._token = signal(localStorage.getItem(this.TOKEN_KEY), ...ngDevMode ? [{ debugName: "_token" }] : []);
    this._user = signal(this.loadUser(), ...ngDevMode ? [{ debugName: "_user" }] : []);
    this.isAuthenticated = computed(() => !!this._token(), ...ngDevMode ? [{ debugName: "isAuthenticated" }] : []);
    this.currentUser = computed(() => this._user(), ...ngDevMode ? [{ debugName: "currentUser" }] : []);
    this.token = computed(() => this._token(), ...ngDevMode ? [{ debugName: "token" }] : []);
  }
  login(request) {
    return this.http.post(`${environment.apiUrl}/auth/login`, request).pipe(tap((res) => {
      if (res.success) {
        const data = res.data;
...
@angular/router/fesm2022/router.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-ODXN7PDN.js.map
